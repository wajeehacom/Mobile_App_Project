...
class MySQLiteHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun getTotalFoodExpensesLastMonth(): Double {
        val db = this.readableDatabase

        // Get the current date
        val calendar = java.util.Calendar.getInstance()

        // Set calendar to the start of the previous month
        calendar.set(java.util.Calendar.DAY_OF_MONTH, 1)
        calendar.add(java.util.Calendar.MONTH, -1)
        val startOfMonth = calendar.time

        // Set calendar to the end of the previous month
        ...
        // Debugging output
        Log.d("Expenses", "Last Month - Start Date: $startDateStr, End Date: $endDateStr")

        // Construct the query to get total food expenses for the previous month
        val query = """
        SELECT SUM($COLUMN_AMOUNT) 
        FROM $TABLE_EXPENSES 
        WHERE $COLUMN_CATEGORY = ? 
        AND $COLUMN_DATE BETWEEN ? AND ?
    """.trimIndent()

        val cursor = db.rawQuery(query, arrayOf("Food", startDateStr, endDateStr))

        var totalFoodExpenses = 0.0
        if (cursor.moveToFirst()) {
            totalFoodExpenses = cursor.getDouble(0) ?: 0.0
        } else {
            Log.d("Expenses", "No food expenses found for the previous month.")
        }
    ...
    fun getTotalFoodExpensesLastWeek(): Double {
        val db = this.readableDatabase

        // Get the current date
        val calendar = java.util.Calendar.getInstance()

        // Set calendar to the start of the previous week (Monday)
        calendar.set(java.util.Calendar.DAY_OF_WEEK, java.util.Calendar.MONDAY)
        calendar.add(java.util.Calendar.WEEK_OF_YEAR, -1)
        val startOfWeek = calendar.time

        // Set calendar to the end of the previous week (Sunday)
        ...
        // Debugging output
        Log.d("Expenses", "Last Week - Start Date: $startDateStr, End Date: $endDateStr")

        // Construct the query to get total food expenses for the last week
        val query = """
        SELECT SUM($COLUMN_AMOUNT) 
        FROM $TABLE_EXPENSES 
        WHERE $COLUMN_CATEGORY = ? 
    ...
    """.trimIndent()
        ...
        if (cursor.moveToFirst()) {
            ...
            Log.d("Expenses", "No food expenses found for the last week.")
        }
